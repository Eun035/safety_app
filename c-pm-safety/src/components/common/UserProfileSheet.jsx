...
const RadarChart = ({ data }) => {
    ...
    return (
        <svg viewBox="-15 -15 130 130" width={110} height={110} className="overflow-visible">
            ...
            {/* Labels */}
            {labels.map((label, i) => {
                const p = getPoint(i * angleStep, center + 6);
                return (
                    <text key={i} x={p.x} y={p.y} textAnchor="middle" dominantBaseline="middle"
                        fontSize="7.5" fill="rgba(255,255,255,0.65)" fontWeight="bold">
                        {label}
                    </text>
                );
            })}
