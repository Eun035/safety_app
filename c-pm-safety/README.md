...
## 🧬 데이터 스키마 철학 (Data Schema Philosophy)
C-Safe의 데이터 스키마는 단순한 사후 주행 로그(Passive Log) 저장이 아닌, **'사용자의 실시간 행동 양식 분석 및 능동적 리스크 교정'**을 위한 컨텍스트 결합형 구조를 지향합니다. 데이터는 다음과 같은 철학 하에 설계됩니다.

*   🎯 **행동 중심성 (Behavior-Centric)**: 디바이스의 물리적 이동 거리보다 운전자가 실시간 위험 요소를 맞닥뜨렸을 때 나타나는 브레이킹, 속도 제어, 경로 선택 등의 '행동 변화'를 정밀하게 포착합니다.
*   🔌 **인프라 동기화 (Infrastructure-Linked)**: 스마트 살균 스테이션(하드웨어)의 도킹 상태 및 플라즈마 에어 샤워 가동 데이터와 실시간 앱 데이터를 동기화하여 공간적 신뢰성을 확보합니다.
*   🗺️ **맥락적 결합 (Contextual Integration)**: 단순 위치 위경도 좌표에 공간 메타데이터(지자체 사고 다발 구역, 교통약자 유도블록 위치, 날씨 등)를 실시간으로 결합하여 리스크의 깊이를 측정합니다.

## 📊 Rider Safety Index (RSI) & 위험 행동 계산 구조
운전자의 주행 신용도를 정량화하는 **Rider Safety Index (RSI)**의 데이터 산출 공식과 가중치 스키마는 다음과 같이 설계되어 있습니다.

### 📐 RSI 산출 공식
$$\text{RSI} = 0.4 \times H + 0.2 \times S + 0.2 \times B + 0.2 \times R$$

### 📋 가중치 및 데이터 스키마 매핑 테이블

| 변수명 | 데이터 소스 및 수집 방식 | 가중치 | 핵심 데이터 필드 (Schema) |
| :--- | :--- | :---: | :--- |
| **H (Helmet)** | 출발 전 스마트 스테이션 도킹 해제 센서 및 모바일 앱 AI 비전 카메라 실시간 인증 데이터 | **40%** | <ul><li>`is_helmet_verified (Bool)`</li><li>`auth_confidence_score (Float)`</li></ul> |
| **S (Speed)** | GPS 수신 속도 및 지자체 세부 도로별(어린이보호구역 등) 제한속도 데이터 매핑 | **20%** | <ul><li>`zone_compliance_rate (Float)`</li><li>`max_over_speed (Float)`</li></ul> |
| **B (Braking)** | 주행 상태 및 위험 상황에 따라 동적으로 조정되는 적응형 센서 샘플링 기반으로 원시 IMU/GPS 데이터를 수집 (스마트폰/디바이스 3축 가속도 및 자이로스코프 센서 연동) | **20%** | <ul><li>`sudden_brake_count (Int)`</li><li>`g_force_magnitude (Float)`</li></ul> |
| **R (Route)** | 고위험 교차로 및 사고 다발 구역 진입 전 AI 음성 넛지에 따른 실시간 우회/서행 이행률 | **20%** | <ul><li>`hazard_avoidance_rate (Float)`</li><li>`nudge_response_time (Int)`</li></ul> |

## 🛡️ 예외 처리 및 장애 극복 전략 (Fault-Tolerance & Fallback Strategy)
C-Safe 플랫폼은 네트워크 및 디바이스 환경의 일시적 장애가 사용자의 안전 사고로 이어지는 것을 방지하기 위해 철저한 **4대 탄력성 폴백(Fallback) 안전장치**를 구축하여 아키텍처적 견고함을 확보하였습니다.

| 장애 유형 | 감지 및 대응 메커니즘 | 폴백 전략 (Fallback Strategy) |
| :--- | :--- | :--- |
| 🛰️ **GPS 신호 불안정** | 위성 수신 장애, 실내/터널 진입으로 인한 좌표 유실 | **최근 안정 좌표 기반 추정 (Dead Reckoning)**<br>마지막으로 기록된 신뢰도 높은 정상 위치 좌표 데이터를 유지하며, 이동 궤적 데이터를 통해 일시적으로 위치를 보정 및 추적. |
| 🌐 **네트워크 통신 장애** | Supabase 서버 연결 장애, 모바일 데이터 음영지역 진입 | **로컬 위험 캐시 (Local Cache Check)**<br>앱 실행 초기에 캐시되어 동기화된 로컬 위험 데이터베이스(`localStorage`)를 즉시 참조하여 실시간 경고 파이프라인 무중단 유지. |
| 🤖 **Edge AI 비전 검증 실패** | 온디바이스 `onnxruntime` 로딩 실패 또는 브라우저 WebGL 미지원 | **보수적 안전 모드 폴백 (Conservative Fallback)**<br>AI 모델 로딩 실패 시 주행을 원천 차단하는 대신, 주행 속도 한계를 초보자 규격(15km/h)으로 강등 강제하고 로깅 데이터 수집 상태로 점진적 전환. |
| 🗣️ **TTS 음성 가이드 실패** | Web Speech API 모듈 지연, 기기 미디어 음소거 상태 | **모바일 햅틱 교대 송출 (Haptic Alternative)**<br>교차로 진입 50m 전 넛지 음성이 정상 출력되지 않을 경우, 모바일 기기의 햅틱 진동(`navigator.vibrate`) 신호를 강하게 교대 송출하여 즉각 위험 통보. |
