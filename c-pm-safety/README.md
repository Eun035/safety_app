...
### 5. 백엔드 및 모바일 호환성 핫픽스 (Hotfixes)
- **Supabase 406 에러 해결**: 안전 퀴즈 통과 시 익명 신규 사용자의 프로필 조회 과정에서 발생하던 `406 Not Acceptable` 에러를 `.maybeSingle()`과 `upsert` 로직으로 유연하게 대체하여, 에러 없이 500P 리워드가 즉각 생성 및 지급되도록 수정했습니다.
- **모바일 맵 터치 이벤트 복구**: 카카오맵의 'P' 주차구역 마커가 스마트폰 터치를 무시하는 현상을 파악하여, 맵 자체의 드래그 이벤트와 충돌하던 커스텀 핸들러를 제거하고 `clickable={true}` 공식 속성과 CSS 최적화로 모바일에서도 원활하게 목적지를 설정할 수 있도록 복구했습니다.
