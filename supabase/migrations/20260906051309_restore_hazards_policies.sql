...
--     (운영 hazards 테이블 0행)
--   - get_nearby_hazards는 SECURITY INVOKER(secdef=false)라 RLS를 우회하지 못한다.
--     따라서 주행 중 위험구역 음성 경고도 함께 죽어 있었다. 안전 기능의 핵심이다.
--
-- 읽기를 공개로 되돌리는 근거:
--   hazards에는 개인정보 컬럼이 없는 도시 공공 안전 정보(급경사·사고다발·도로파손)이고,
--   앱의 모든 사용자(익명 로그인 포함)가 위험구역을 봐야 안전 경고가 성립한다.
--   제보자 신원 등 민감 정보는 이 테이블에 저장하지 않는다(아래 후속 과제 참고).
