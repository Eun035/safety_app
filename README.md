...
## 🗓️ 최근 업데이트 — 2026-08-29

- ✅ **운영 DB `feedbacks` 테이블 복구** — 테이블이 없어 **탈퇴 사유 수집이 계속 실패**하던 것을 수정. Supabase CLI가 다른 프로젝트에 링크돼 있던 것도 바로잡고 마이그레이션 이력 baseline 정리
- ✅ **지역 전환 버그 수정** — 천안↔아산을 바꿔도 PM 스테이션·장소 검색이 **이전 지역 좌표 기준**으로 남던 문제
- ✅ **기상 위험 감지 위치 기반 전환** — 천안 고정 좌표 → **현재 위치 기준 + 10분 주기 갱신**. API 실패 시 30% 확률로 **가짜 경고를 만들어내던 기믹 제거**
- ✅ **추천 가입 연결 수정** — PostgREST UPDATE에 order/limit이 안 먹어 409로 **조용히 실패**하던 문제
- ✅ **초기 임계 경로 gzip 479KB → 314KB** — QR 지연 로딩, i18n 언어별 분리, 벤더 청크 분리
