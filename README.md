...
## 🗓️ 최근 업데이트 — 2026-09-06

**🔒 보안 (2026-08-29에 남겨둔 과제 완료)**
- ✅ **`profiles` 전체 공개 읽기 차단** — anon 키만으로 전 사용자의 닉네임·나이·포인트·추천관계를 덤프하고 realtime으로 관전까지 가능했다 → 본인 행만
- ✅ **관리자 역할 도입** — 클라이언트 하드코딩 비밀번호(`admin1234`) 제거, **이메일 로그인 + 서버 판정**(`admin_users` / `is_admin()`)
- ✅ **SECURITY DEFINER 함수 권한 정리** — anon 실행 전면 회수, `search_path` 고정. 추천 보상 RPC가 인자를 무검증 신뢰하던 문제도 수정

**🐛 예정에 없던 발견**
- ✅ **시민 위험 제보 기능이 죽어 있었다** — `hazards` 정책이 0개라 RLS가 전면 차단. 제보 저장·지도 표시·**주행 중 음성 경고**까지 멈춰 있었다. 정책 복구 + `location` 자동 채움 트리거 추가
- ✅ **대시보드 실측/목업 구분** — 조회 실패해도 하드코딩 값으로 그럴듯하게 채워져 위 고장이 드러나지 않았다. 출처 배지·경고 배너 추가

📄 상세: **[SESSION_2026-09-06.md](SESSION_2026-09-06.md)** · 관리자 계정 설정: **[supabase/README.CLI.md](supabase/README.CLI.md)** 7절

<details>
<summary>이전 업데이트 — 2026-08-29 ~ 08-30</summary>


**🔒 보안**
- ✅ **임의 프로필 변조 차단** — `profiles` RLS가 열려 있어 **누구나 남의 포인트·닉네임을 바꿀 수 있었다**(재현 확인). 포인트가 보상 재화라 보상 체계 전체가 무의미했던 상태 → 본인 행으로 제한
- ✅ **탈퇴 사유 노출 차단** — `feedbacks` 공개 조회 정책 제거
- ⏸ 남은 것: 관리자 역할 부재(클라이언트 하드코딩 비밀번호) — 별도 작업 예정

**🐛 결함 수정**
- ✅ **운영 DB `feedbacks` 테이블 복구** — 테이블이 없어 **탈퇴 사유 수집이 계속 실패**하던 것을 수정. Supabase CLI가 다른 프로젝트에 링크돼 있던 것도 바로잡고 마이그레이션 이력 baseline 정리
- ✅ **지역 전환 버그** — 천안↔아산을 바꿔도 PM 스테이션·장소 검색이 **이전 지역 좌표 기준**으로 남던 문제
- ✅ **기상 위험 감지 위치 기반 전환** — 천안 고정 좌표 → **현재 위치 기준 + 10분 주기 갱신**. API 실패 시 30% 확률로 **가짜 경고를 만들어내던 기믹 제거**
- ✅ **추천 가입 연결** — PostgREST UPDATE에 order/limit이 안 먹어 409로 **조용히 실패**하던 문제
- ✅ **거리 단위 정합화** — 주행거리·평균속도 **1000배**, 헬멧 스테이션 **반경 필터가 항상 실패**하던 문제
- ✅ **CO₂ 계수 일원화** — 화면마다 0.2/0.13로 제각각이던 것을 근거 있는 **0.100 kg/km**(순 절감량) 단일 출처로

**⚡ 성능·품질**
- ✅ **초기 임계 경로 gzip 479KB → 272KB (−43%)** — 벤더 분리, QR·i18n·시트 18종 지연 로딩
- ✅ **lint 에러 58 → 0, 경고 14 → 0** — ESLint 설정 오탐(`jsx-uses-vars` 누락) 포함

📄 상세: **[SESSION_2026-08-29.md](SESSION_2026-08-29.md)** · 로컬 DB 가이드: **[supabase/README.CLI.md](supabase/README.CLI.md)**

</details>

<details>
<summary>이전 업데이트 — 2026-07-29</summary>

- ✅ **라이드 서머리 미표시 버그 수정** — 결제 '나중에'/닫기 시 뒤 체인이 끊겨 서머리가 안 뜨던 문제
- ✅ **공유 카드 개인정보 보호** — SNS 공유 이미지에서 **도착 장소(목적지) 제외**
- ✅ **스피드 히트 루트 + 베지어 곡선** — 경로를 **속도별 색 그라디언트** + ⚡마커로, GPS 라인을 **부드러운 S자 곡선**(chill)으로
- ✅ **경로 선 스타일 선택** — 굵기 + 선 종류(실선/점선/도트/이중선) 칩 선택
- ✅ **Supabase CLI 도입** — 로컬↔운영 마이그레이션 연동, 로컬 baseline 복원, **Scoop 네이티브 설치** 전환, `npm run dev` 시 **로컬 DB 자동 기동**(predev)

📄 상세: **[SESSION_2026-07-29.md](SESSION_2026-07-29.md)**

</details>

<details>
<summary>이전 업데이트 — 2026-07-05</summary>
