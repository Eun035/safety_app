            ...
            <div className="bg-[#12161b] w-full sm:max-w-md rounded-t-[2rem] sm:rounded-[2rem] shadow-2xl border border-orange-500/30 overflow-hidden relative animate-in slide-in-from-bottom flex flex-col pt-2 pb-8 px-6">
                ...
                {isScanning ? (
                    <div className="flex flex-col items-center justify-center py-10">
                        ...
                        <h3 className="text-xl font-black text-white mb-2">{t('dg_waiting')}</h3>
                        <p className="text-sm text-orange-400 font-bold animate-pulse">{t('dg_tap')}</p>
                    </div>
                ) : (
                    <>
                        ...
                        <div className="bg-gradient-to-r from-orange-950/80 to-[#1e1410] border border-orange-500/30 rounded-2xl p-4 mb-6 relative overflow-hidden">
                            ...
                            <p className="text-[10px] font-black uppercase text-orange-500 tracking-widest mb-1 relative z-10">{t('dg_reward')}</p>
                            <p className="text-md font-bold text-white relative z-10 leading-snug">
                                {t('dg_24h')}<br />
                                {t('dg_discount_pre')}<span className="text-orange-400 font-black">{station.discountOffer}</span>{t('dg_discount_post')}
                            </p>
                        </div>
                        ...
                        >
                            <Scan size={20} className="font-black" /> {t('dg_nfc_return')}
                        </button>
                    </>
                )}
            </div>
