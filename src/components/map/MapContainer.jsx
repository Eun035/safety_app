        ...
        <div className="relative w-full h-full bg-[#0a1118] overflow-hidden">
            ...
            >
                ...
                >
                    ...
                    {/* 🌋 보행자 스트레스 존 레이어 — App.jsx STRESS_ZONES 단일 소스 (천안·아산 모두) */}
                    {/* 현재 지역 외 zone은 흐리게 표시 ("모두 표시 + 현재 지역 강조" 정책) */}
                    {showStressLayer && stressZones.map(zone => {
                        const isCurrentRegion = (zone.region || 'cheonan') === currentRegion;
                        return (
                            <Circle
                                key={`stress-${zone.id}`}
                                ...
                                strokeWeight={2}
                                strokeColor="#f97316"
                                strokeOpacity={isCurrentRegion ? 0.8 : 0.35}
                                fillColor="#f97316"
                                fillOpacity={isCurrentRegion ? 0.2 : 0.08}
                            />
                        );
                    })}
                </Map>
